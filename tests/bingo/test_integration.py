...
class TestBingoQueryIntegration:
    ...
    def test_complex_query_with_multiple_conditions(self):
        ...
        benzene = "c1ccccc1"
        aspirin_smarts = "[#6]-[#6](=[#8])-[#8]-[#6]1:[#6]:[#6]:[#6]:[#6]:[#6]:1-[#6](=[#8])-[#8]"
        stmt = select(self.compounds).where(
            or_(
                self.compounds.c.structure.substructure(benzene),
    ...
    def test_join_with_bingo_conditions(self):
        ...
        stmt = select(
            self.compounds.c.name,
            suppliers.c.name.label('supplier_name')
        ).select_from(
            self.compounds.join(
                suppliers, 
                self.compounds.c.id == suppliers.c.compound_id
            )
        ).where(
            self.compounds.c.structure.substructure(benzene)
        )
...
class TestBingoQueryVariations:
    ...
    def test_all_function_methods(self):
        ...
        similarity_expr = bingo_func.similarity(self.compounds.c.structure, ethanol, 0.7)
